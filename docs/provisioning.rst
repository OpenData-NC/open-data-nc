...
Opendata is deployed on the following stack.
...
- ``ALLOWED_HOSTS`` has been set in ``opendata/settings/<environment>.py``
...
RabbitMQ user will be named opendata and the vhost will be named opendata_<environment>
...
Additionally you will need to configure the project settings for django-celery::
    ...
    BROKER_URL = 'amqp://opendata:%s@localhost:5672/opendata_staging' % os.environ['BROKER_PASSWORD']

You will also need to add the ``BROKER_URL`` to the ``opendata.settings.production`` so
